...
public class InvestimentoDAO implements Dao<Investimento> {
    ...
    public Investimento get(int id) {
        ...
        try {
            PreparedStatement sql = conexao.getConexao().prepareStatement("SELECT * FROM Investimento WHERE id_investimento = ?");
            sql.setInt(1, id);
    ...
    public void insert(Investimento investimento) {
        ...
        try {
            PreparedStatement sql = conexao.getConexao()
                    .prepareStatement("INSERT INTO Investimento (id_cliente, id_fundo, vr_cota_incio, vr_investido, qtd_cotas) VALUES (?,?,?,?,?)");
            sql.setInt(1, investimento.getIdCliente());
    ...
    public void update(Investimento investimento) {
        ...
        try {
            PreparedStatement sql = conexao.getConexao()
                    .prepareStatement("UPDATE Investimento SET id_cliente=?, id_fundo=?, vr_cota_incio=?, vr_investido=?, qtd_cotas=? WHERE id_investimento=?");
            sql.setInt(1, investimento.getIdCliente());
            sql.setInt(2, investimento.getIdFundo());
            sql.setFloat(3, investimento.getVrCotaInicio());
            sql.setFloat(4, investimento.getVrInvestido());
            sql.setFloat(5, investimento.getQtdCotas());
            sql.setInt(5, investimento.getIdInvestimento());
            sql.executeUpdate();
    ...
    public void delete(int id) {
        ...
        try {
            PreparedStatement sql = conexao.getConexao()
                    .prepareStatement("DELETE FROM Investimento WHERE id_investimento = ?");
            sql.setInt(1, id);
            sql.executeUpdate();
    ...
    private Investimento createFromResultSet(ResultSet resultado) {
        Investimento investimento = new Investimento();
        try {
            investimento.setIdInvestimento(Integer.parseInt(resultado.getString("id_investimento")));
            investimento.setIdCliente(Integer.parseInt(resultado.getString("id_cliente")));
